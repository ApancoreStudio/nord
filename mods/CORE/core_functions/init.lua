...
-- Split
core_functions.split = function(inputstr, sep)
	if sep == nil then
		sep = "%s"
	end
	local t={}
...
core_callback.register_on_press = function(func)
	table.insert(core_callback.registered_on_press, func)
end
...
core_callback.register_on_release = function(func)
	table.insert(core_callback.registered_on_release, func)
end
...
core_callback.register_on_hold = function(func)
	table.insert(core_callback.registered_on_hold, func)
end
...
core_callback.register_on_wield_index_change = function(func)
	table.insert(core_callback.registered_on_wield_index_change, func)
end
...
core_callback.reset_hold_time = function(player, control_name)
	...
	core_callback.players[player_name].controls[control_name][2] = minetest.get_us_time()/MICROSECONDS
end
...
minetest.register_on_joinplayer(function(player)
	...
	core_callback.players[player_name] = {
		controls = {
			jump = {false, 0},
			right = {false, 0},
			left = {false, 0},
...
minetest.register_on_leaveplayer(function(player)
	...
	core_callback.players[player_name] = nil
end)
